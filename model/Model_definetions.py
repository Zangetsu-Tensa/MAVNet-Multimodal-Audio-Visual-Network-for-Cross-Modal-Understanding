import tensorflow as tf
from tensorflow.keras import layers, models, applications
import librosa
import numpy as np
import cv2
import os


def build_audio_model():
    input_audio = tf.keras.Input(shape=(128, 128, 1))  

    x = layers.Conv2D(32, (3, 3), activation='relu', padding='same')(input_audio)
    x = layers.MaxPooling2D((2, 2))(x)
    x = layers.Conv2D(64, (3, 3), activation='relu', padding='same')(x)
    x = layers.MaxPooling2D((2, 2))(x)
    x = layers.Conv2D(128, (3, 3), activation='relu', padding='same')(x)
    x = layers.MaxPooling2D((2, 2))(x)
    # x = layers.GlobalAveragePooling2D()(x)

    x = layers.Reshape((32, -1))(x)  

    x = layers.LSTM(128, return_sequences=False)(x)
    x = layers.Dense(128, activation='relu')(x)

   
    return tf.keras.Model(inputs=input_audio, outputs=x)

def build_visual_model():
    base_model = applications.ResNet50(input_shape=(224, 224, 3), include_top=False, weights='imagenet')
    x = layers.GlobalAveragePooling2D()(base_model.output)
    x=layers.Dense(128,activation='relu')(x)
    visual_output = layers.Dense(128, activation='relu')(x)
    return models.Model(inputs=base_model.input, outputs=visual_output)

def build_fusion_model(num_classes):
    audio_model = build_audio_model()
    visual_model = build_visual_model()

    audio_input = audio_model.input
    visual_input = visual_model.input

    audio_feat = audio_model.output
    visual_feat = visual_model.output

    merged = layers.concatenate([audio_feat, visual_feat])
    x = layers.Dense(128, activation='relu')(merged)
    x = layers.Dropout(0.3)(x)
    x = layers.Reshape((32, -1))(x)
    x = layers.LSTM(128, return_sequences=False)(x)
    output = layers.Dense(num_classes, activation='softmax')(x)

    model = models.Model(inputs=[audio_input, visual_input], outputs=output)
    return model